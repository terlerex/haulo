...
def migrate(con: sqlite3.Connection) -> None:
    ...
    add = {
        ...
        "stock_items": {
            ...
            # différents. Organisme par défaut PSA, mais saisissable librement.
    ...
    # Studio photo retiré (jamais vraiment utilisé, la photo au téléphone/
    # scanner suffit) : les tables ne sont plus créées par init(), on nettoie
    # celles qui existent encore sur une base pré-existante. L'ordre compte
    # (studio_pairs référence studio_jobs). N'affecte jamais media/CARD_MEDIA/
    # STOCK_MEDIA (pipeline Pillow des items/fiches, entièrement séparé).
    con.execute("DROP TABLE IF EXISTS studio_pairs")
    con.execute("DROP TABLE IF EXISTS studio_jobs")
